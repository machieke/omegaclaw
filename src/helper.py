...
def receive_latest_message():
    ...
    for channel in order:
        ...
        if msg:
            filtered = _filter_incoming_by_auth(msg, channel)
            if filtered:
...
def _tool_skill_from_user_message(user_msg, max_send_chars):
    ...
    persona_control = _persona_control_skill_from_user_message(user_msg, max_send_chars)
...
def _ensure_capability_policy_ready():
    ...
    with _CAPABILITY_POLICY_LOCK:
        ...
        registrations = [
            ...
            ("list-personas", "List available personas", False),
            ("use-persona", "Switch active persona", True),
            ("current-persona", "Read active persona", False),
